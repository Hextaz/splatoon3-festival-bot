...
const addMatchToHistory = (team1Name, team2Name) => {
    matchHistoryManager.addMatchToHistory(team1Name, team2Name);
};

const calculateOpponentScore = (teamName, potentialOpponent) => {
    return matchHistoryManager.calculateOpponentScore(teamName, potentialOpponent);
};

const getTeamMatchHistory = (teamName) => {
    return matchHistoryManager.getTeamMatchHistory(teamName);
};
...
async function startMatchSearch(interaction, team, isTestMode = false) {
    ...
    setTimeout(() => checkWaitingTeam(searchEntry), 30000);
}

// Commencer la recherche de match sans nécessiter d'interaction (pour les équipes virtuelles)
function startVirtualTeamSearch(team, guild = null) {
    // Vérifier si le festival est actif
    const festival = getCurrentFestival();
    if (!festival || !festival.isActive) {
        console.log(`Tentative de recherche annulée: aucun festival actif`);
    ...
    if (existingSearch) {
        console.log(`[TEST] L'équipe ${team.name} est déjà en recherche depuis ${Math.floor((Date.now() - existingSearch.startTime) / 1000)} secondes`);
        return false;
    }
    
    // Vérifier si l'équipe est en match
    if (team.currentOpponent) {
        console.log(`[TEST] L'équipe ${team.name} est déjà en match contre ${team.currentOpponent}`);
        return false;
    }
    
    // Vérifier si l'équipe a au moins 4 membres
    if (team.members.length < 4) {
        console.log(`[TEST] L'équipe ${team.name} n'a que ${team.members.length} membres et ne peut pas rechercher`);
        return false;
    }

    // IMPORTANT: Ne pas chercher de match immédiat ici!
    // Ajouter simplement à la file d'attente
    if (guild) {
        team.storedGuild = guild; // Stocker la référence à la guild dans l'équipe
    }
    ...
    // IMPORTANT: Ne pas marquer l'équipe comme occupée ici, cela sera fait lors du match
    searchingTeams.push(searchEntry);
    console.log(`[TEST] L'équipe ${team.name} a été ajoutée à la file d'attente (${searchingTeams.length} équipes en attente)`);
    
    return true;
}

// Vérifier une équipe après son temps d'attente minimum
async function checkWaitingTeam(searchEntry) {
    // Vérifier si l'équipe est toujours en recherche
    ...
    const match = findMatch(searchEntry.team);
...
function findMatch(team) {
    ...
    const availableTeamsWithScores = availableTeams.map(t => {
        ...
        return {
            ...t,
            waitTime: waitTime,
            score: calculateOpponentScore(team.name, { ...t, waitTime })
        };
    });
    ...
    return selectOpponentWithWeighting(team, availableTeamsWithScores);
}
...
function selectOpponentWithWeighting(team, availableTeamsWithScores) {
    ...
    console.log(`${team.name} attend depuis ${Math.round(waitTimeMinutes * 100) / 100} minutes`);
    ...
    try {
        ...
        for (let i = 0; i < teamsToProcess.length; i++) {
            ...
            if (entry.team.busy || entry.team.currentOpponent) {
                cleanupSearch(entry.team.name);
                continue;
            }
            
            const match = findMatch(entry.team);
...
async function createMatch(interaction, team1, team2, onMatchCreated = null) {
    ...
    return await withTeamLock([team1.name, team2.name], 'createMatch', async () => {
        ...
        addMatchToHistory(updatedTeam1.name, updatedTeam2.name);
...
function cleanupSearch(teamName) {
...
setInterval(async () => {
    ...
    try {
        ...
        if (searchingTeams.length > 0 && 
            ...
            console.log(`État de la file d'attente: ${searchingTeams.length} équipes en recherche`);
            lastQueueLog = now;
        ...
        if (!team1 || !team2 || team1.busy || team2.busy || 
            ...
            if (!team2 || team2.busy || team2.currentOpponent) {
                searchingTeams.splice(searchingTeams.indexOf(entry2), 1);
            }
            return;
        }
...
function finishMatch(team1Name, team2Name) {
...
function repairInconsistentStates() {
...
module.exports = {
    ...
    createMatch,
    finishMatch,
    getSearchingTeams: () => searchingTeams,
    startVirtualTeamSearch,
    resetSearchQueue,
